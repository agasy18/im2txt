...
def input_fn(dataset, feature_extuctor, is_training, cache_dir, batch_size, max_train_epochs):
    with working_dir(cache_dir, True):
        ...
        d = contrib.data.Dataset.zip(fd, cd).map(merge, num_threads=4, output_buffer_size=batch_size * 4)
        if is_training:
...
def feature_dataset():
    ...
    return contrib.data.TFRecordDataset([cache_file_name]).map(_parse), feature_size
...
def create_feature_records(dataset, feature_extuctor, is_training):
    ...
    def ext_f(sess, id, jpeg, height, width):
        f: np.ndarray = sess.run([features], feed_dict={jpg_data: jpeg}),
        s_features = {
        ...
        int_box[0] = f.size
        return tf.train.Example(features=tf.train.Features(feature=s_features))
